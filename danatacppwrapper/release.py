"""Release data for danata-cpp-wrapper.

When danata-cpp-wrapper is imported a number of steps are followed to determine
the version information.
   ...
   1) If the release is not a development release (dev=False), then version
      ...
      downloadable release of danata-cpp-wrapper since setup.py creates it during
      packaging/installation.  However, version.py might not exist if one
      is running danata-cpp-wrapper from the mercurial repository.  In the event that
...
danata-cpp-wrapper, we must make sure write_versionfile() does not overwrite the
...
def write_versionfile():
    ...
    text = '''"""
Version information for danata-cpp-wrapper, created during installation.
...
# Was danata-cpp-wrapper built from a development version? If so, remember that the major
...
MAINTAINER = "danata-cpp-wrapper Developers"
MAINTAINER_EMAIL = "danatacppwrapper-discuss@googlegroups.com"
URL = 'http://danata-cpp-wrapper.github.io/'
DOWNLOAD_URL = 'T.B.D.'
